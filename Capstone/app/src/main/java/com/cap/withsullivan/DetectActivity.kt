...
class DetectActivity : AppCompatActivity() {
    ...
    val binding by lazy { ActivityDetectBinding.inflate(layoutInflater) }
...
//        val binding = ActivityDetectBinding.inflate(layoutInflater)
    ...
    private fun openCamera() {
        try {
            ...
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(this@DetectActivity, "카메라를 열지 못했습니다.", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private val mSessionPreviewStateCallback = object : CameraCaptureSession.StateCallback() {
        ...
        override fun onConfigureFailed(session: CameraCaptureSession) {
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(this@DetectActivity, "카메라 구성 실패", Toast.LENGTH_SHORT).show()
            }
        }
    }
        ...
        try {
            ...
            val captureListener = object : CameraCaptureSession.CaptureCallback() {
                override fun onCaptureCompleted(
                    ...
                    Handler(Looper.getMainLooper()).post {
                        Toast.makeText(this@DetectActivity, "사진이 촬영되었습니다", Toast.LENGTH_SHORT)
                            .show()
                    }
                    when (cameracheck) {
